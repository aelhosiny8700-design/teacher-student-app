...
class _UploadContentScreenState extends State<UploadContentScreen> {
  ...
  Future<void> _upload() async {
    ...
    final item = ContentItem(
      ...
      // 🔴 ملاحظة: تم ضبطه ليدعم نظام الإشعارات (تأكد أن موديل المحتوى يدعم isRead أو يتم تخزينه في الـ Map مباشرة)
    );

    try {
      // تعديل إضافي لضمان إرسال حقل isRead: false مع البيانات لخدمة الـ Badge
      await FirestoreService().addContentCustomMap({
        'title': _titleController.text.trim(),
        'description': _descController.text.trim(),
        'type': _type,
        'url': result.url!,
        ...
        'isRead': false, // 🔴 هذه العلامة الحمراء ستزيد العدد عند الطالب فوراً
      });
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('تم رفع المحتوى بنجاح! وزادت تنبيهات الطلاب بنجاح 🎉'),
            backgroundColor: Colors.green,
          ),
        );
      }
    } catch (e) {
      // في حال لم تكن دالة الـ CustomMap موجودة، نستعمل الدالة الأصلية
      try {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('تم رفع المحتوى بنجاح!'),
              backgroundColor: Colors.green,
            ),
          );
        }
        ...
        if (mounted) {
          setState(() {
            _uploading = false;
            _error = 'اترفع الملف لكن حصل خطأ في حفظ البيانات: $err';
          });
        }
      }
    }
